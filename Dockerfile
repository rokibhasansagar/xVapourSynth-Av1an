...
RUN <<-'EOL'
	...
	sed -i -e '/pkgver=/c\pkgver=3.5.r103.g34532bda1' -e '/pkgrel=/c\pkgrel=3' -e '/conflicts=/d' -e "/build() {/a\    export CFLAGS+=' -Wno-unused-parameter -Wno-unused-result' CXXFLAGS+=' -Wno-unused-parameter -Wno-unused-result'" -e '/-Wno-dev/i\        -DCMAKE_ASM_NASM_FLAGS=-w-macro-params-legacy \\' -e '/EXTRA_LINK_FLAGS/a\        -DENABLE_CLI=ON \\' -e "/build() {/i\prepare() {\n  sed -i '/set(X265_BUILD 2[0-9][0-9])/c\set(X265_BUILD 199)' x265_git/source/CMakeLists.txt\n}\n" x265-git/PKGBUILD
